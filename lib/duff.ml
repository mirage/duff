...
let derive v buf off =
  ...
  let v = Uint32.(((v lsl 8) lor (of_int (unsafe_get_uint8 buf off)))
...
let pp_entry : entry Fmt.t = fun ppf entry ->
  Fmt.pf ppf "{ @[<hov>offset = %d;@ \
              hash = %lxu;@] }"
...
let unsafe_make source =
  ...
  do
    ...
    then
      begin
        unpacked.(!rev - 1) <- { unpacked.(!rev - 1) with offset = !idx + _window };
        decr entries;
      end
...
let bigstring_iteri ?(start = 0) f c =
  ...
  then invalid_arg "iter" ;

  let idx = ref start in

  while !idx < len
  do match f !idx (unsafe_get_uint8 c !idx) with
    | `Stay -> ()
    | `Cont -> incr idx
    | `Move add ->
      let max = len - !idx in
...
let same (src, src_off) (dst, dst_off) =
  ...
  while src_off + !idx < src_len
        && dst_off + !idx < dst_len
        && unsafe_get_uint8 src (src_off + !idx) == unsafe_get_uint8 dst (dst_off + !idx)
  do incr idx done;
    ...
    let (copy_off, copy_len), current_hash =
      ...
      then
        ...
        List.fold_left
          ...
          (copy_off, copy_len)
          (* TODO: optimize? *)
          (Array.get index.hash Uint32.(to_int (current_hash land index.mask))
           |> List.filter (fun (entry : entry) -> Uint32.compare entry.hash current_hash = 0)),
        current_hash
